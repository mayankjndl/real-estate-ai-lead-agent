...
function outcomeCell(lead) {
    if (lead.visit_date) {
        return `<span class="outcome-visit">📅 Visit: ${lead.visit_date}</span>`;
    }
    if ((lead.score || '').toLowerCase() === 'high') {
        return `<span class="outcome-followup">✅ High intent — follow-up pending</span>`;
    }
    if (lead.phone) {
        return `<span class="outcome-contact">📞 Contact captured</span>`;
    }
