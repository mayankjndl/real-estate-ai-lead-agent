...
def run_dynamic_checks(session_id: str, tc: TestCase, bot_replies: List[str]) -> Dict[str, Any]:
    ...
    with _Session() as db:
        ...
        fup_row = db.execute(text("SELECT * FROM follow_up_states WHERE session_id = :sid"), {"sid": session_id}).fetchone()
    ...
    if not lead:
        return report # Cannot check further if lead wasn't created
    ...
    if mentioned_locs:
        expected_loc = mentioned_locs[-1] # The LAST mentioned location (Mind-change logic)
        actual_loc = str(lead.get("location", "")).lower()
        report[f"extracted_location_{expected_loc}"] = expected_loc in actual_loc

    # Infer Last Budget
    budgets = re.findall(r'(\d+(?:\.\d+)?\s*(?:lakhs?|l|cr|crores?))', combined_user_text)
    if budgets:
    ...
    if tc.category == "HOT":
        # Hot leads should trigger high intent and visit dates
        visit_keywords = ["visit", "saturday", "sunday", "tomorrow", "today", "schedule"]
        if any(k in combined_user_text for k in visit_keywords):
            report["visit_scheduled"] = lead.get("visit_date") is not None

        # They should be marked HOT by ML layer (due to visit + full info + phone number)
        report["marked_hot"] = lead.get("lead_temperature") == "hot"

        if fup:
            # If they are fully qualified, follow up status becomes 'completed'
            report["followup_status_correct"] = fup.get("follow_up_status") in ["completed", "stopped"]

    elif tc.category == "COLD":
        # Cold leads shouldn't be marked hot, and followups should stay active or stopped (if user replied last)
...
def build_test_cases() -> List[TestCase]:
    raw_cases = [
        ...
        ("W26", "WARM", "Negotiation", ["Looking for 2BHK in Baner 80 lakhs", "Can we negotiate the price?"]),
        ("W27", "WARM", "Relocation", ["Moving from Bangalore for work, need 2BHK near Hinjewadi, budget 70 lakhs"]),
        ("W28", "WARM", "Budget Range", ["Looking for 2BHK in Baner, budget between 60 and 75 lakhs"]),
        ...
        ("H06", "HOT", "Reactivated Hot", ["Hi, I was in touch earlier. Now I'm ready to buy. 2BHK in Baner, 80 lakhs, can I visit this week?"]),
        ("H07", "HOT", "Budget Upward", ["I'm Varun. Looking for 2BHK in Kothrud, budget 65 lakhs", "Wait, my budget just got approved for 90 lakhs. Can I get 3BHK?", "Yes, 3BHK. Can I visit Saturday?"]),
    ...
    return [TestCase(r[0], r[1], r[2], r[3]) for r in raw_cases]
...
# ─── RUNNER LOGIC ───────────────────────────────────────────────────────────
async def run_single_conversation(client: httpx.AsyncClient, tc: TestCase, base_url: str, api_key: str) -> Result:
    ...
    if tc.id in RETURNING_TESTS:
        ...
        session_id = f"+9199{uuid.uuid4().hex[:8]}"
    ...
    return Result(tc.id, tc.category, tc.label, session_id, passed, details, bot_replies, error, duration)


async def run_stress_test(base_url: str, api_key: str, category_filter: str = None):
    ...
    async with httpx.AsyncClient() as client:
        for i, tc in enumerate(cases, 1):
            ...
            res = await run_single_conversation(client, tc, base_url, api_key)
            results.append(res)
...
def generate_report(results: List[Result]):
    total = len(results)
    ...
    print(f"\n{'='*70}")
    print(f"📊 FINAL AUTOMATION HARDENING REPORT")
    print(f"{'='*70}")
    print(f"Total Scenarios Tested : {total}")
    print(f"Passed                 : {passed}")
    print(f"Failed                 : {failed}")
    print(f"Pass Rate              : {pass_rate:.1f}%")
    print(f"Target 95% Criteria    : {'✅ MET' if pass_rate >= 95.0 else '❌ NOT MET'}")

    # Save to JSON
    out_file = f"test_task3_maitri_results.json"
...
if __name__ == "__main__":
    ...
    asyncio.run(run_stress_test(args.base_url, args.api_key, args.category))